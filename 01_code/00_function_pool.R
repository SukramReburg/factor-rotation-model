...
check_packages <- function(pkg_list, quiet = FALSE) {
  ...
  if (any(is_installed)) {
    installed_versions <- vapply(
      pkgs[is_installed],
      function(p) {
        v <- tryCatch(utils::packageVersion(p), error = function(e) NA)
        if (is.na(v[1])) NA_character_ else as.character(v)
      },
      character(1)
    )
...
load_packages <- function(pkg_list, quietly = FALSE) {
  ...
  for (pkg in pkgs) {
    if (requireNamespace(pkg, quietly = TRUE)) {
      library(pkg, character.only = TRUE, quietly = quietly, warn.conflicts = FALSE)
      loaded <- c(loaded, pkg)
    } else {
...
plot_trend_fits <- function(
  ...
  hit_idx <- if (length(hit_idx)) hit_idx[1] else NA_integer_
  
  # base plot (extra space on the right for legend)
  op <- par(mar = c(4, 4, 3, 20), xaxs = "i", yaxs = "i", xpd = NA)
  on.exit(par(op), add = TRUE)
  
  plot(y, type = "l", lwd = 1, col = "grey40",
       xlab = xlab, ylab = ylab, main = main)
  
  # overlay fits
  for (i in seq_len(n)) {
    lines(fits_ord[[i]],
          lwd = if (!is.na(hit_idx) && i == hit_idx) 3 else 2,
          col = cols[i])
  }
  if (!is.na(hit_idx)) {
  ...
  legend("topright", inset = c(-0.7, 0), xpd = NA, bty = "n",
         legend = leg_labels, col = leg_cols, lty = 1, lwd = leg_lwd,
         title = "Series / df")
  invisible(NULL)
}
...
fill_trend_edges <- function(y, trend) {
  ...
  n <- length(y)
  if (n == 0L) return(trend)
  
  # first and last non-NA indices in trend
  idx <- which(!is.na(trend))
  if (!length(idx)) return(trend)  # nothing to anchor to — leave as is
